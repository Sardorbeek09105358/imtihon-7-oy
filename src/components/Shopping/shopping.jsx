...
const Shopping = () => {
  ...
  const removeCart = (index) => {
    ...
    // Update localStorage if you want to persist the changes
          ...
          <div className='flex justify-center pt-[250px]'>
            <div className='w-full flex flex-wrap gap-10 mx-auto justify-center flex-col items-center'>
              <div className='w-[68%] flex justify-between items-center'>
                <h2 className='capitalize text-[30px] font-bold'>shopping cart: {products.length} items</h2>
              ...
              {products.map((item, index) => (
                <div key={index} className="w-[70%] flex h-[260px] items-center justify-evenly  mb-10 border-2 rounded-md border=[#999] pl-10">
                  ...
                  <button onClick={() => removeCart(index)} className='capitalize text-[24px] flex items-center gap-3 text-red-600'><i className="fa-solid fa-xmark"></i>remove</button>
                  <button onClick={handleAdd}  className='capitalize text-[22px] w-[200px] h-10'> to home page</button>
                </div>
              ))}
            </div>
          </div>
